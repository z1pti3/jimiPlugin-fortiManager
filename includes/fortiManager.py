#author: Akshay Mane
#gitHub: https://github.com/akshaymane920/pyFortimanagerAPI
...
class _fortiManager:
   ...
   # Firewall Virtual IP objects
    ...
    def update_firewall_vip_object(self, name, data):
        session = self.login()
        payload = \
            {
                "method": "update",
        ...
        payload = repr(payload)
        update_object = session.post(url=self.base_url, data=payload, verify=self.verify)
        return update_object.json()["result"]

    # Firewall IP Pool objects
    def get_firewall_ippool_objects(self, name=False):
        url = f"pm/config/adom/{self.adom}/obj/firewall/ippool"
        if name:
            url = f"pm/config/adom/{self.adom}/obj/firewall/ippool/{name}"
        session = self.login()
        payload = \
            {
                "method": "get",
    ...
    def update_firewall_ippool_object(self, name, data):
        session = self.login()
        payload = \
            {
                "method": "update",
...
def getAdoms(fortiManagerClass):
    adoms = []
    response = fortiManagerClass.get_adoms()
    for data in response[0]["data"]:
        adoms.append(data["name"])
    return adoms

def getObjects(fortiManagerClass,objectType):
    objectTypes = {
        "address" : fortiManagerClass.get_firewall_address_objects,
        "vip" : fortiManagerClass.get_firewall_vip_objects,
        "ippool" : fortiManagerClass.get_firewall_ippool_objects
    }
    result = objectTypes[objectType]()[0]
    if result["status"]["code"] == 0:
        return result["data"]
    return result

def getObject(fortiManagerClass,objectType,objectName):
    objectTypes = {
        "address" : fortiManagerClass.get_firewall_address_objects,
        "vip" : fortiManagerClass.get_firewall_vip_objects,
        "ippool" : fortiManagerClass.get_firewall_ippool_objects
    }
    result = objectTypes[objectType](objectName)[0]
    if result["status"]["code"] == 0:
        return result["data"]
    return result

def setObject(fortiManagerClass,objectType,objectName,jsonFieldValue):
    if objectType == "address":
        result = fortiManagerClass.update_firewall_address_object(objectName,**jsonFieldValue)[0]
    elif objectType == "vip":
        result = fortiManagerClass.update_firewall_vip_object(objectName,jsonFieldValue)[0]
    elif objectType == "ippool":
        result = fortiManagerClass.update_firewall_ippool_object(objectName,jsonFieldValue)[0]
    if result["status"]["code"] == 0:
        return True
    return False
